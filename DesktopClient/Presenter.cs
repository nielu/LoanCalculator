...
namespace DesktopClient
{
    public class Presenter
    {
        ...
        public Presenter(string serverUrl, IVIew view)
        {
            View = view;
        ...
        public async Task GetPayments(Decimal TotalAmount, UInt16 NumberOfYears, UInt16 LoanTypeID)
        {
            ...
            if (response.IsSuccessStatusCode)
                View.PaymentList = await response.Content.ReadAsAsync<List<Payment>>();
            else
                throw new Exception();
        }
        ...
        public async Task GetLoanTypes()
        {
            ...
            if (response.IsSuccessStatusCode)
                View.LoanTypeList = await response.Content.ReadAsAsync<List<LoanType>>();
            else
